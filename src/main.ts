...
const dateFormatter = new Intl.DateTimeFormat('zh-Hant-TW', { dateStyle: 'medium' });
...
function localizeError(error: unknown, fallback: string): string {
  if (error instanceof TimelineParseError) {
    switch (error.reason) {
      case 'malformed-json':
        return '這不是有效或完整的 JSON 檔。';
      case 'legacy-format':
        return '這是較舊的 Google Takeout 格式。請改從手機匯出時間軸資料。';
      case 'raw-signals-only':
        return '這個匯出檔只有原始定位，沒有已整理的旅程。';
      case 'unsupported-format':
        return '時間軸 JSON 必須是陣列，或包含 semanticSegments。';
      case 'no-usable-locations':
        return '這個時間軸匯出檔沒有可用的定位點。';
    }
  }
...
function rebuildRawSignalProcessing(): boolean {
  ...
  if (limit !== null && (!Number.isFinite(limit) || limit < 0)) {
    setSettingsError('請輸入大於或等於 0 的準確度上限，或留空。');
...
function currentPeriodLabel(): string {
  if (rawSignalsToggle.checked) return '原始定位資料';
  if (exactDateToggle.checked) {
...
function refreshActionAvailability(points = currentPoints()): void {
  ...
  if (!compatibilityChecked) {
    createButton.title = '正在檢查瀏覽器的影片支援。';
  } else if (!encodingSupported) {
    createButton.title = '產出 MP4 需要支援 WebCodecs 與 H.264 編碼的瀏覽器，例如 Chrome、Edge 或 Safari 16.4 以上。';
  } else if (!hasJourney) {
    createButton.title = '請選擇至少包含兩個不同地點的期間。';
  } else {
...
function updateSelection(): void {
  ...
  if (points.length === 0) {
    selectionSummary.textContent = '這段期間沒有定位點';
  } else if (points.length === 1) {
    selectionSummary.textContent = '僅 1 個定位點 · 請擴大期間';
  } else if (distanceKm <= 0) {
    selectionSummary.textContent = `${points.length.toLocaleString('zh-Hant-TW')} 個定位點 · 沒有移動`;
  } else {
    const estimate = rawSignalsToggle.checked ? '估計 ' : '約 ';
    const ignored = rawSignalsToggle.checked && rawSignalProcessing?.rejectedCount
      ? ` · 已忽略 ${rawSignalProcessing.rejectedCount.toLocaleString('zh-Hant-TW')} 個雜訊或不準的點`
      : '';
    selectionSummary.textContent = `${points.length.toLocaleString('zh-Hant-TW')} 個定位點 · ${estimate}${Math.round(distanceKm).toLocaleString('zh-Hant-TW')} 公里${ignored}`;
  }
  prepared = null;
  selectedSignature = '';
...
async function getPreparedJourney(signal?: AbortSignal): Promise<PreparedJourney> {
  ...
  progressLabel.textContent = '正在準備地圖';
...
function requireMapConsent(): boolean {
  ...
  setSettingsError('請先確認地圖隱私說明，才會向 CARTO 請求地圖圖磚。');
  mapConsent.focus();
  return false;
}
...
function applyTimeline(data: unknown, sourceName: string, useRawOnly = false): void {
  ...
  const timezoneNote = allPoints.some((point) => point.timeZoneMissing)
    ? ' · 缺少時區，已保留原始路線順序'
    : '';
  const sourceNote = useRawOnly ? ' · 原始定位後援' : '';
  fileStatus.textContent = `${sourceName} · ${allPoints.length.toLocaleString('zh-Hant-TW')} 個有效點，從 ${months[0].label} 到 ${months.at(-1)?.label}${sourceNote}${timezoneNote}`;
  updateSelection();
}
...
async function loadTimeline(file: File): Promise<void> {
  ...
  try {
    ...
    if (error instanceof TimelineParseError && error.reason === 'raw-signals-only' && rawPoints.length > 0) {
      ...
      fileStatus.textContent = '只找到原始定位資料';
...
fileInput.addEventListener('change', async () => {
  ...
  try {
    ...
    fileStatus.textContent = '無法載入時間軸';
    setError(localizeError(error, '無法讀取所選檔案。'));
...
sampleButton.addEventListener('click', async () => {
  ...
  try {
    ...
    applyTimeline(parseTimelineText(await response.text()), '虛構範例');
  } catch (error) {
    settingsCard.classList.add('hidden');
    fileStatus.textContent = '無法載入範例';
    setError(localizeError(error, '無法載入虛構範例。'));
    previewCard.classList.remove('hidden');
  }
});
...
openGoogleMapsButton.addEventListener('click', () => {
  ...
  rawOnlyDialog.close();
  settingsCard.classList.add('hidden');
  fileStatus.textContent = '請先在 Google 地圖確認到訪與行程出現，再重新匯出並載入新檔。';
});
...
rawOnlyDialog.addEventListener('cancel', () => {
  pendingRawOnlyImport = null;
  settingsCard.classList.add('hidden');
  fileStatus.textContent = '已取消載入原始定位';
});
...
createButton.addEventListener('click', async () => {
  ...
  try {
    ...
    progressLabel.textContent = '正在產出 MP4';
    const blob = await createJourneyMp4(canvas, journey, {
      durationSeconds: Number(durationSelect.value),
      title: titleInput.value.trim() || '我的旅程',
      periodLabel: currentPeriodLabel(),
      signal: exportController.signal,
      onProgress: (fraction) => {
        progress.value = fraction;
        progressLabel.textContent = `正在產出 MP4 ${Math.round(fraction * 100)}%`;
      },
    });
    ...
    progressLabel.textContent = `影片已就緒 · ${(blob.size / 1_000_000).toFixed(1)} MB`;
    ...
    if (exportController.signal.aborted || (error instanceof DOMException && error.name === 'AbortError')) {
      progressLabel.textContent = '已取消產出影片';
      progress.value = 0;
    } else {
      setError(localizeError(error, '產出影片失敗。'));
      progressLabel.textContent = '無法產出影片';
    }
  } finally {
...
void canCreateMp4(canvas.width, canvas.height).then((supported) => {
  ...
  compatibilityStatus.textContent = supported
    ? '這個瀏覽器可以產出 H.264 MP4。'
    : '目前只能預覽。產出 MP4 需要支援 WebCodecs 與 H.264 的瀏覽器，例如最新版 Chrome、Edge 或 Safari。';
  refreshActionAvailability();
});
